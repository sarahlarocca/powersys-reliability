...
pfe.all <- read.table('edgeremovals.csv',header=TRUE)
names(pfe.all) <- c('id','numfail','LCSG','D','E','EN','ENE','CL','PCL','PNS','DC','AC')
pf.all <- data.frame(pfe.all)				# original dataset
AC.all <- pfe.all$AC					# original AC results
