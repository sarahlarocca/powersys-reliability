...
function [Efail,EfailInit,runTime,cpuTime] = calcEfficPower_V3(A,failures,failType,compFromNode,compToNode)
    ...
    for j = 1:numFailElements
        switch failType
            case 'N'                    % for node failures
                ...
                Efail(i,j) = mean(dFailureInv(dFailureInv~=Inf));
...
dFailureInv = 1./dFailure;
EfailInit = mean(dFailureInv(dFailureInv~=Inf));